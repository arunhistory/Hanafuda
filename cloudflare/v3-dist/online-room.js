...
export class HanafudaOnlineRoom {
    ...
    async action(body) {
        ...
        if ((await this.state.storage.get("connectionsReady")) !== true)
            return json({ ok: false, code: "WAITING_FOR_CONNECTION" }, 409);
    ...
    async postmatch(body) {
        ...
        try {
            if (choice === "same") {
                ...
                await this.broadcastState(newStatus);
    ...
    async alarm() {
        ...
        if (cd && cd <= now && (await this.state.storage.get("connectionsReady")) !== true)
