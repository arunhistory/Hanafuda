...
const REGRESSION_GUARDED_SOCKETS=new WeakSet<WebSocket>();
const REGRESSION_CHECKED_SOCKETS=new WeakSet<WebSocket>();

function regressionStoredDealer():FirstDealer|null{
  try{
    const raw=localStorage.getItem(REGRESSION_SETTINGS_KEY);if(!raw)return null;
    const value=Number(JSON.parse(raw)?.firstDealer);
    return value===-1||value===0||value===1?value as FirstDealer:null;
  }catch{return null;}
}
function regressionRestoreDealer(){
  const stored=regressionStoredDealer();
  if(stored!==null)settings.firstDealer=stored;
}
function regressionDealerButton(value:FirstDealer,label:string){
...
function regressionPatchDealerUi(){
  if(currentScreen()!=='cpu-setup')return;
  const row=app.querySelector<HTMLElement>('.dealer-choice-row');if(!row)return;
  ...
  row.querySelectorAll<HTMLElement>('[data-cpu-dealer]').forEach(el=>{
...
function regressionRequestUrl(input:RequestInfo|URL){
...
async function regressionVerifyOnlineSocket(s:OnlineSession,ws:WebSocket){
  if(REGRESSION_CHECKED_SOCKETS.has(ws)||session!==s||s.socket!==ws)return;
  REGRESSION_CHECKED_SOCKETS.add(ws);
  try{
    const result=await api('/api/online/status',{roomCode:s.roomCode,token:s.token});
    if(session!==s||s.socket!==ws||ws.readyState===WebSocket.OPEN)return;
  ...
  }catch{toast('オンライン接続を確認できませんでした。再接続します。');try{ws.close();}catch{}}
}
function regressionGuardOnlineSocket(){
  const s=session;if(!s||s.kind!=='online'||!s.socket)return;
  const ws=s.socket;if(REGRESSION_GUARDED_SOCKETS.has(ws))return;
  REGRESSION_GUARDED_SOCKETS.add(ws);
  const timer=window.setTimeout(()=>{if(session===s&&s.socket===ws&&ws.readyState!==WebSocket.OPEN)void regressionVerifyOnlineSocket(s,ws);},7000);
...
function regressionPatchScreens(){
  ...
  if(currentScreen()==='settings')main?.classList.add('settings-screen-expanded');
  if(currentScreen()==='cpu-setup')main?.classList.add('cpu-setup-expanded');
...
document.addEventListener('click',event=>{
  const target=event.target instanceof Element?event.target.closest<HTMLElement>('[data-cpu-dealer]'):null;
  if(!target||!target.closest('#app'))return;
  const value=Number(target.dataset.cpuDealer);if(value!==-1&&value!==0&&value!==1)return;
  settings.firstDealer=value as FirstDealer;saveSettings();regressionPatchDealerUi();
});