function dealerLabelV3(v:FirstDealer){return v===0?"あなた":v===1?"相手":"ランダム";}
...
async function releaseCpuAfterReadyV3(){
  ...
  snapshot=ready.data.snapshot as Snapshot;
...
async function startCpuSequencedV3(){
  ...
  try{
    ...
    session={kind:"cpu",sessionId:String(started.data.sessionId),token:String(started.data.token),version:Number(started.data.version),mode:settings.mode,rounds:settings.rounds,koiEnabled:settings.koiEnabled,firstDealer,modeSessionId,modeSessionToken};
    snapshot=started.data.snapshot as Snapshot;
...
document.addEventListener("click",event=>{
  const target=event.target instanceof Element?event.target.closest<HTMLElement>("[data-action='start-cpu']"):null;
  if(!target)return;
