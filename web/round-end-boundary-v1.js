(()=>{
  ...
  function isFinalRoundSettlement(){
    if(!snapshot||snapshot.phase!==5)return false;
  ...
  function purgeTransient(){
    if(typeof window.__hanafudaPurgeTransientMatchOverlays==="function"){
      window.__hanafudaPurgeTransientMatchOverlays();
      return;
    }
  ...
  sendAction=async function(action,payload={}){
    if(action!=="next_round"||session?.kind!=="cpu"||!isFinalRoundSettlement())return baseSendAction(action,payload);
    ...
    try{
      ...
      if(!result.ok||!result.data?.ok)throw new Error(result.data?.code||"FINALIZE_FAILED");
      ...
      if(!result.data.snapshot)throw new Error("FINAL_SNAPSHOT_MISSING");
  ...
  window.__hanafudaRoundEndBoundaryVersion="1";
})();
