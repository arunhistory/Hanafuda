...
export class HanafudaCpuSession {
  ...
  async transition(){
    ...
    const events=[{actor:"system",snapshot:created.data.snapshot,version:Number(created.data.version),actionEvent:null}];
    if(Number(created.data.snapshot?.turn)===1&&CPU_PHASES.has(Number(created.data.snapshot?.phase))){const cpu=await this.runCpu(events);if(!cpu.ok)return cpu.response;}
    const finalEvent=events[events.length-1];
