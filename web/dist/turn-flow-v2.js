...
(() => {
    ...
    function candidatesFor(handIndex) {
        if (!snapshot || !Number.isInteger(handIndex) || !Number.isInteger(snapshot.hand?.[handIndex]))
            return [];
    ...
    function clearStage(restore = true) {
        ...
        if (restore && snapshot && currentScreen() === "match")
            renderMatch();
    }
    function paintStage() {
        if (stagedHand === null || !snapshot)
            return;
        ...
        if (status)
            status.textContent = candidates.length ? "取る場札を選んでください。手札を押し直すと選び直せます。" : "同じ月の札がありません。「ここに出す」を選んでください。手札は選び直せます。";
    }
    async function commit(fieldIndex) {
        if (stagedHand === null || committing || !inPlayerPlayPhase())
            return;
        const handIndex = stagedHand;
        const candidates = candidatesFor(handIndex);
        if (fieldIndex !== null && !candidates.includes(fieldIndex))
            return;
        if (fieldIndex === null && candidates.length !== 0)
            return;
        committing = true;
        stagedHand = null;
        try {
            ...
            if (fieldIndex !== null && snapshot && snapshot.turn === playerSeat() && snapshot.phase === 2) {
                ...
                if (pending.includes(fieldIndex))
                    await runtimeWindow.sendAction("capture", { fieldIndex });
            }
        }
        finally {
            committing = false;
    ...
    app.addEventListener("click", event => {
        ...
        if (field && stagedHand !== null) {
            ...
            if (candidatesFor(stagedHand).includes(index))
                void commit(index);
            return;
        }
    ...
    runtimeWindow.startCpu = async function () {
        ...
        try {
            ...
            if (settings.mode !== "impossible") {
                ...
                modeSessionId = String(mode.data.sessionId);
                modeSessionToken = String(mode.data.token);
            }
    ...
    async function revealRoundDealSequentially() {
        ...
        for (let i = 0; i < max; i++) {
            if (opponent[i])
                sequence.push(opponent[i]);
            if (player[i])
                sequence.push(player[i]);
            if (field[i])
                sequence.push(field[i]);
        }
    ...
    runtimeWindow.sendAction = async function (action, payload = {}) {
        ...
        if (busy || roundTransitionPending || !session || !snapshot)
            return;
        ...
        try {
            ...
            if (!nextSnapshot)
                throw new Error("NEXT_ROUND_SNAPSHOT_MISSING");
            snapshot = nextSnapshot;
            roundHistory = [];
            currentRound = nextSnapshot.roundIndex;
            renderMatch();
            if (!settings.skipNormalAnimations)
                await revealRoundDealSequentially();
            emitAudioHook("deal");
            await showReadyGate();
    ...
    runtimeWindow.chooseKoi = async function (continueKoi) {
        if (koiDecisionPending)
            return;
        koiDecisionPending = true;
        try {
            for (let guard = 0; busy && guard < 240; guard++)
                await delay(25);
            if (busy) {
