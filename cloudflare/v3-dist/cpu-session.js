...
export class HanafudaCpuSession {
    ...
    async init(body) {
        ...
        if (!validOpaqueToken(token) || !mode || rounds === null || typeof koiEnabled !== "boolean" || !validOpaqueToken(modeSessionId) || !validOpaqueToken(modeSessionToken))
...
export async function routeCpu(req, env, url) {
    ...
    if (url.pathname === "/api/cpu/start") {
        ...
        const modeSessionId = String(body?.modeSessionId ?? ""), modeSessionToken = String(body?.modeSessionToken ?? "");
        if (!mode || rounds === null || typeof koiEnabled !== "boolean" || !validOpaqueToken(modeSessionId) || !validOpaqueToken(modeSessionToken))
