...
function decorateStateModal() {
    if (modal !== "state" || !snapshot)
        return;
    const panel = app.querySelector(".modal.subview");
    if (!panel || panel.querySelector("[data-state-public-detail='1']"))
        return;
...
function lockForcedTransitionControl() {
    const button = app.querySelector("[data-action='accept-transition']");
    if (!button)
        return;
