...
assert.match(ts,/--deck-offset/,'deck extraction must receive an explicit landscape-local pixel offset');
assert.match(ts,/--deck-mid-offset/,'deck extraction must use a browser-compatible explicit midpoint');
assert.match(ts,/--capture-x/,'capture motion must receive a board-relative pixel distance to the capture rail');
...
assert.match(css,/deck-pull-landscape/,'deck draw must use an explicit horizontal landscape path');
