...
class Assets {
    ...
    async load() {
        ...
        if (!response.ok)
            throw new Error("ASSET_REGISTRY_LOAD_FAILED");
    ...
    path(id) {
        ...
        if (!item?.path)
            throw new Error(`ASSET_NOT_FOUND:${id}`);
        return `../${item.path}`;
    }
    card(card) {
        const item = this.registry?.assets["cards.generated"];
        if (!item?.pattern)
            throw new Error("CARD_PATTERN_NOT_FOUND");
...
function phaseLabel(s) {
    ...
    if (s.phase === 2 || s.phase === 3)
        return s.turn === playerSeat() ? "取る場札を選んでください" : "相手が取得札を選択中";
    if (s.phase === 4)
        return s.turn === playerSeat() ? "役が成立しました" : "相手がこいこいを判断中";
    if (s.phase === 5)
        return "局の精算";
    if (s.phase === 6)
        return "対局終了";
...
function playerSeat() { return session?.kind === "online" ? session.seat : 0; }
function opponentSeat() { return (1 - playerSeat()); }
...
} el.innerHTML = queuedToasts.map(x => `<div class="toast">${escapeHtml(x)}</div>`).join(""); }
function topbar(title, backEnabled = true) { return `<header class="topbar">${backEnabled ? '<button class="icon-button" data-action="back" aria-label="戻る">←</button>' : '<span></span>'}<h1>${escapeHtml(title)}</h1><button class="icon-button" data-action="home" aria-label="ホーム">⌂</button></header>`; }
