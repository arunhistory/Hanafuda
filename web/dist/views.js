...
async function render() {
    ...
    if (screen === "home")
        ...
        await renderCloudDocumentScreen("terms", "利用規約");
    else if (screen === "credits")
        await renderCloudDocumentScreen("credits", "クレジット");
    else if (screen === "licenses")
        await renderCloudDocumentScreen("licenses", "ライセンス");
...
async function renderCloudDocumentScreen(key, title) {
    const expected = key;
    ...
    try {
        ...
        if (currentScreen() !== expected)
            return;
        const state = app.querySelector("#cloud-content-state");
        if (!state)
            return;
        if (!documentValue.available) {
            state.textContent = "現在、本文は未登録です。";
            return;
        }
        if (typeof documentValue.body !== "string") {
            state.textContent = "本文を取得できませんでした。";
            return;
        }
...
function backs(count) { const src = assets.path("cards.back"); return Array.from({ length: Math.min(count, 8) }, () => `<img class="card card-back" src="${src}" alt="裏向きの札" draggable="false">`).join(""); }
function capturedHtml(cards) { return cards.map(c => cardImg(c)).join(""); }
function handHtml(s) {
...
function opponentLabel() { if (session?.kind === "cpu") {
    if (session.mode === "impossible")
        return hiddenFirstEncounter ? "▧▒░█?▒" : "人知不能";
    return "CPU / " + modeLabel(session.mode);
} return "対戦相手"; }
...
function renderMatchOverlay() {
    ...
    if (onlineReconfigureState !== "none")
        return renderOnlineReconfigure();
...
function renderOnlineReconfigure() {
    if (onlineReconfigureState === "guest")
        return `<div class="modal-layer"><section class="modal"><h2>条件を再設定</h2><p>ホストが次の対局条件を設定しています。</p></section></div>`;
...
function renderPauseModal() {
    if (!modal)
        return "";
    if (modal === "pause")
        return `<div class="modal-layer"><section class="modal"><h2>対局中メニュー</h2><div class="modal-list"><button class="menu-item" data-modal="close"><strong>再開</strong></button><button class="menu-item" data-modal="yaku"><strong>役の得点と組み合わせ</strong></button><button class="menu-item" data-modal="state"><strong>現状</strong></button><button class="menu-item" data-modal="history"><strong>履歴</strong><span>現在局のみ</span></button><button class="menu-item" data-modal="rules"><strong>ルール確認</strong></button><button class="menu-item" data-modal="settings"><strong>設定</strong></button><button class="menu-item danger" data-modal="giveup"><strong>諦める</strong></button></div></section></div>`;
    const backButton = '<button class="secondary" data-modal="pause">戻る</button>';
    if (modal === "yaku")
        return `<div class="modal-layer"><section class="modal subview"><h2>役の得点と組み合わせ</h2>${yakuTable()}<div class="screen-actions">${backButton}</div></section></div>`;
    if (modal === "state")
        return `<div class="modal-layer"><section class="modal subview"><h2>現状</h2>${stateTable()}<div class="screen-actions">${backButton}</div></section></div>`;
    if (modal === "history")
        return `<div class="modal-layer"><section class="modal subview"><h2>現在局の履歴</h2>${roundHistory.length ? `<ol class="history-list">${roundHistory.map(h => `<li>${escapeHtml(h)}</li>`).join("")}</ol>` : "<p>まだ手順履歴はありません。</p>"}<div class="screen-actions">${backButton}</div></section></div>`;
    if (modal === "rules")
...
function stateTable() {
    if (!snapshot)
        return "";
    const s = snapshot, [myScore, oppScore] = perspectiveScores(s), [myCap, oppCap] = perspectiveCaptured(s);
...
function bindGlobalActions() {
    ...
    if (rounds)
        rounds.onchange = () => { settings.rounds = Number(rounds.value); saveSettings(); };
    const koi = app.querySelector("#koi-enabled");
    if (koi)
        koi.onchange = () => { settings.koiEnabled = koi.checked; saveSettings(); };
    const skip = app.querySelector("#skip-animations");
    if (skip)
        skip.onchange = () => { settings.skipNormalAnimations = skip.checked; saveSettings(); };
    app.querySelector("[data-action='start-cpu']")?.addEventListener("click", () => void startCpu());
    app.querySelector("[data-action='online-create']")?.addEventListener("click", () => void createOnlineRoom());
    app.querySelector("[data-action='online-inspect']")?.addEventListener("click", () => void inspectOnlineRoom());
    app.querySelector("[data-action='online-join']")?.addEventListener("click", () => void joinOnlineRoom());
    app.querySelector("[data-action='online-random']")?.addEventListener("click", () => void randomOnline());
}
function bindMatchActions() {
    app.querySelectorAll("[data-hand-index]").forEach(b => b.onclick = () => void sendAction("play", { handIndex: Number(b.dataset.handIndex) }));
    app.querySelectorAll("[data-field-index]").forEach(b => b.onclick = () => void sendAction("capture", { fieldIndex: Number(b.dataset.fieldIndex) }));
    app.querySelector("[data-action='pause']")?.addEventListener("click", () => { modal = "pause"; renderMatch(); });
    app.querySelectorAll("[data-modal]").forEach(el => el.onclick = () => { const value = el.dataset.modal; modal = value === "close" ? null : value; if (value === "close")
        renderMatch();
    else
        renderMatch(); });
    const matchSkip = app.querySelector("#match-skip-animations");
    if (matchSkip)
        matchSkip.onchange = () => { settings.skipNormalAnimations = matchSkip.checked; saveSettings(); };
    app.querySelector("[data-action='confirm-giveup']")?.addEventListener("click", () => void closeMatch(true));
    app.querySelector("[data-action='koi-continue']")?.addEventListener("click", () => void chooseKoi(true));
    app.querySelector("[data-action='koi-finish']")?.addEventListener("click", () => void chooseKoi(false));
    app.querySelector("[data-action='next-round']")?.addEventListener("click", () => void nextRound());
    app.querySelector("[data-action='accept-transition']")?.addEventListener("click", () => void beginImpossibleTransition());
    app.querySelector("[data-action='finish-home']")?.addEventListener("click", () => void closeMatch(true));
    app.querySelector("[data-action='cpu-reconfigure']")?.addEventListener("click", () => void cpuPostmatch("reconfigure"));
    app.querySelector("[data-action='cpu-same']")?.addEventListener("click", () => void cpuPostmatch("same"));
    app.querySelectorAll("[data-postmatch]").forEach(el => el.onclick = () => void onlinePostmatch(el.dataset.postmatch));
    app.querySelector("[data-action='apply-online-reconfigure']")?.addEventListener("click", () => void applyOnlineReconfigure());
}
