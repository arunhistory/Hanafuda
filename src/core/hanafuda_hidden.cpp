...
static const int MAX_NODES=420000;
...
static int strategic_count(int p,uint32_t flag){
  int n=0;
  for(int i=0;i<game_captured_n(p);i++)if(card_flags(game_captured_card(p,i))&flag)n++;
  return n;
}

static int live_card_value(int id,int p){
  if(id<0||id>=48)return 0;
  const uint32_t f=card_flags(id);
  int v=0;
  if(f&H_LIGHT)v+=90+strategic_count(p,H_LIGHT)*24;
  if(f&H_TANE)v+=28+strategic_count(p,H_TANE)*7;
  if(f&H_RIBBON)v+=24+strategic_count(p,H_RIBBON)*7;
  if(f&H_KASU)v+=6+strategic_count(p,H_KASU)*2;
  if(f&H_AKATAN)v+=45;
  if(f&H_AOTAN)v+=45;
  if(f&H_SAKE)v+=60;
  if(id==24||id==37||id==21)v+=65;
  if(id==2||id==6||id==10)v+=55;
  if(id==22||id==34||id==38)v+=55;
  if(id==9||id==29||id==33)v+=55;
  return v;
}

static int position_heuristic(){
  const int opp=1-hidden_actor;
  const int ownScore=captured_score(hidden_actor);
  const int oppScore=captured_score(opp);
  int value=(ownScore-oppScore)*12000;
  value+=(game_captured_n(hidden_actor)-game_captured_n(opp))*180;
  for(int i=0;i<game_hand_n(hidden_actor);i++)value+=live_card_value(game_hand_card(hidden_actor,i),hidden_actor)*12;
  for(int i=0;i<game_hand_n(opp);i++)value-=live_card_value(game_hand_card(opp,i),opp)*9;
  const int rem=game_deck_remaining();
  for(int i=0;i<rem&&i<8;i++){
    const int card=game_deck_card_relative(i);
    const int drawer=(game_turn()+i)%2;
    value+=(drawer==hidden_actor?1:-1)*live_card_value(card,drawer)*4;
  }
  if(game_phase()==H_KOI&&game_turn()==hidden_actor)value+=game_offered_score()*9000;
  if(game_koi_used())value+=(ownScore-oppScore)*2500;
  return value;
}
...
static int terminal_utility(){
  ...
  const int points=game_last_round_points();
  if(winner==hidden_actor){
    return 60000000+ownGain*240000+points*12000;
  }
  if(winner==opp){
    return -180000000-oppGain*300000-points*15000;
  }
  return -3000000+position_heuristic()/4;
}
...
static int search_hidden_choice(int depth){
  ...
  int found=0;
  if(phase==H_PLAY){
    const int n=game_hand_n(hidden_actor);
    for(int i=0;i<n;i++){
      if(!copy_state_out(search_states[depth]))return position_heuristic();
      const int rc=game_play_hand(hidden_actor,i);
      if(rc==0){
    ...
    for(int i=0;i<n;i++){
      ...
      if(!copy_state_out(search_states[depth]))return position_heuristic();
      const int rc=game_choose_capture(hidden_actor,fieldIndex);
      if(rc==0){
        const int v=hidden_search(depth+1);
        if(!found||v>best){best=v;found=1;}
      }
    ...
    for(int choice=0;choice<=1;choice++){
      if(!copy_state_out(search_states[depth]))return position_heuristic();
      const int rc=game_koi_decision(hidden_actor,choice);
      if(rc==0){
        const int v=hidden_search(depth+1);
        if(!found||v>best){best=v;found=1;}
      }
  ...
  return found?best:position_heuristic();
}
...
static int hidden_search(int depth){
  ...
  int v=position_heuristic()-5000000;
