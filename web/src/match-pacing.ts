let matchRecapQueue:Promise<void>=Promise.resolve();
...
function actionRecapRows(event:ActionEvent){
  ...
  if(event.capturedCards?.length)rows.push(`<div class="captured"><span>取得</span><div class="recap-cards">${event.capturedCards.slice(0,4).map(card=>cardImg(card)).join("")}</div></div>`);
...
function queueActionRecap(event:ActionEvent){
  matchRecapQueue=matchRecapQueue.then(()=>showActionRecap(event)).finally(()=>{
    if(!app.querySelector(".match-action-recap"))matchRecapBlocking=false;
  });
}
...
window.addEventListener("hanafuda-audio-hook",event=>{
  ...
  if(detail?.name==="card-action"&&detail.event&&!settings.skipNormalAnimations)queueActionRecap(detail.event);
});
