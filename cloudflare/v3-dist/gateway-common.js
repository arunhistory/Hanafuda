...
export async function sha256Hex(s) { const d = new Uint8Array(await crypto.subtle.digest("SHA-256", new TextEncoder().encode(s))); return Array.from(d, b => b.toString(16).padStart(2, "0")).join(""); }
export function timingSafe(a, b) { if (!a || !b || a.length !== b.length)
...
export function roomCode() { const a = new Uint8Array(6); crypto.getRandomValues(a); let s = ""; for (const b of a)
    s += ROOM_ALPHABET[b % ROOM_ALPHABET.length]; return s; }
export function parseRuleSet(v) {
    if (!v || typeof v !== "object" || Array.isArray(v))
        return null;
    const keys = Object.keys(v);
    if (keys.some(k => k !== "rounds" && k !== "koiEnabled"))
        return null;
    const rounds = parseRounds(v.rounds);
    if (rounds === null || typeof v.koiEnabled !== "boolean")
        return null;
    return { rounds, koiEnabled: v.koiEnabled };
}
export function ruleKey(r) { return `${r.rounds}:${r.koiEnabled ? 1 : 0}`; }
