...
let mobileCanvasTimer:number|undefined;
...
function syncMobileCanvas(){
  ...
  if(!isMobileOrTablet()){
    ...
    lastCanvasWidth=0;lastCanvasHeight=0;lastPortrait=null;
    return;
  }
...
function scheduleMobileCanvasSync(immediate=false){
  if(mobileCanvasTimer!==undefined)window.clearTimeout(mobileCanvasTimer);
  if(immediate){syncMobileCanvas();return;}
