(()=>{
  'use strict';
  type BgmKind='none'|'normal'|'impossible';
  type AudioOptions={loop?:boolean;preload?:HTMLMediaElement['preload']};
  type AudioHookDetail={name?:unknown};
  ...
  let bgmKind:BgmKind='none';
  let bgm:HTMLAudioElement|null=null;
  let lastCorrupted=false;
  let finalVisible=false;

  function audio(src:string,{loop=false,preload='none'}:AudioOptions={}){
    const a=new Audio();
    a.src=src;a.loop=loop;a.preload=preload;a.crossOrigin='anonymous';
    return a;
  }
  ...
  function stopBgm(){
    ...
    bgm=null;bgmKind='none';
  }
  function setBgm(kind:BgmKind){
  ...
  function playSe(src:string,volume=1){
    const a=audio(src,{preload:'auto'});a.volume=volume;safePlay(a);return a;
  }
  function playCollapsePair(){
    if(!userActivated)return;
    const a=audio(SOURCES.collapse,{preload:'auto'}),b=audio(SOURCES.collapseMain,{preload:'auto'});
    a.volume=.9;b.volume=.92;
  ...
  function matchState(){
    ...
    if(!match)return {active:false,corrupted:false};
  ...
  function syncPresentation(){
    ...
    if(state.active){
      if(state.corrupted&&!lastCorrupted)playSe(SOURCES.impossibleUnlock,.95);
    ...
    if(nowFinal&&!finalVisible)playSe(SOURCES.finalSettlement,.95);
  ...
  window.addEventListener('hanafuda-audio-hook',event=>{
    ...
    if(name==='impossible-collapse')playCollapsePair();
  });

  const app=document.querySelector<HTMLElement>('#app');
  if(app)new MutationObserver(syncPresentation).observe(app,{subtree:true,childList:true,attributes:true,attributeFilter:['class']});
