...
async function inspectOnlineRoom(){
  const input=app.querySelector<HTMLInputElement>("#room-code");if(!input)return;const code=input.value.trim().toUpperCase();
...
async function joinOnlineRoom(){
  if(!inspectedRoom||busy)return;busy=true;try{const r=await api("/api/online/join",{roomCode:inspectedRoom.code});if(!r.ok||!r.data?.ok)throw new Error(r.data?.code||"ROOM_JOIN_FAILED");await startOnlineSession(inspectedRoom.code,r.data.guestToken,1,r.data.rules,r.data);}catch(e){toast(e instanceof Error?e.message:"参加できません");}finally{busy=false;}
}
async function randomOnline(){
  if(busy)return;busy=true;try{const rules=onlineRulesFromUi();let r=await api("/api/online/random",{rules});if(!r.ok||!r.data?.ok)throw new Error(r.data?.code||"MATCHMAKING_FAILED");const ticket=r.data.queueTicket;toast("対戦相手を待っています…");for(let i=0;i<24&&!r.data.matched;i++){await delay(5000);r=await api("/api/online/random",{ticket});if(!r.ok||!r.data?.ok)throw new Error(r.data?.code||"MATCHMAKING_FAILED");}if(!r.data.matched)throw new Error("MATCHMAKING_TIMEOUT");await startOnlineSession(r.data.roomCode,r.data.token,r.data.seat==="guest"?1:0,r.data.rules,null);}catch(e){toast(e instanceof Error?e.message:"マッチングに失敗しました");}finally{busy=false;}
}
