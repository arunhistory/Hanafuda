...
const checks=[
  ...
  ['beginner has a dedicated novice policy',cpuSessionSrc.includes('mode==="beginner"')&&cpuSessionSrc.includes('handIndex:randomIndex(count)')],
  ['beginner capture choices are deliberately random among legal matches',cpuSessionSrc.includes('pending[randomIndex(pending.length)]')],
  ['amateur and stronger modes still use engine CPU profiles',cpuSessionSrc.includes('result=await this.engineAction({action:"cpu_step",actor:1})')],
