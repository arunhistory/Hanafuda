...
(() => {
    ...
    function scheduleIdle(callback) {
        if (typeof runtimeWindow.requestIdleCallback === "function")
            runtimeWindow.requestIdleCallback(callback, { timeout: 5000 });
        else
            window.setTimeout(callback, 1200);
    }
    function warmFinalBackgroundWhenIdle() {
        if (finalBgWarmScheduled)
            return;
    ...
    function warmImpossibleClearAssetsWhenIdle() {
        if (impossibleClearWarmScheduled)
            return;
    ...
    function revealImpossibleClearReturnHint() {
        if (!impossibleClearActive)
            return;
        impossibleClearReturnEnabled = true;
        const screen = app.querySelector(".impossible-clear-screen");
        screen?.classList.add("return-enabled");
        const hint = app.querySelector(".impossible-clear-return-hint");
        if (hint) {
    ...
    function isImpossibleSpecialVictory(s) {
    ...
    runtimeWindow.renderMatch = function () {
        if (snapshot?.phase === 6) {
            if (isImpossibleSpecialVictory(snapshot))
                return renderImpossibleClear();
            endImpossibleClear();
        ...
        if (snapshot && matchInteractionReady)
            warmFinalBackgroundWhenIdle();
        if (hiddenFirstEncounter)
            warmImpossibleClearAssetsWhenIdle();
        if (koiChoiceCommitted) {
            app.querySelectorAll(".modal-layer").forEach(layer => { if (layer.querySelector(".koi-choice"))
                layer.remove(); });
        }
        return out;
    };
    runtimeWindow.chooseKoi = async function (continueKoi) {
        if (koiChoiceCommitted || busy)
            return;
        if (!snapshot || snapshot.phase !== 4 || snapshot.turn !== playerSeat())
            return;
    ...
    app.addEventListener("click", event => {
        if (!impossibleClearActive || !impossibleClearReturnEnabled)
            return;
        const target = event.target;
        if (!(target instanceof Element) || !target.closest(".impossible-clear-screen"))
            return;
