...
const clearIndex=nextRoundBlock.indexOf('clearRoundSettlementOverlay()');
const shuffleIndex=nextRoundBlock.indexOf('await showShuffle(false)');
const apiIndex=nextRoundBlock.indexOf('const result=await api("/api/cpu/action"');
...
assert.ok(clearIndex>=0&&shuffleIndex>clearIndex,'the prior round settlement overlay must disappear before shuffle starts');
assert.ok(apiIndex>shuffleIndex,'the server must not create the next round until shuffle has finished');
assert.ok(commitNextIndex>apiIndex,'the next-round snapshot must enter the DOM only after the post-shuffle server transition succeeds');
...
assert.match(js,/function clearRoundSettlementOverlay\(\)/,'later-round transition must explicitly remove the prior winner/settlement overlay');
