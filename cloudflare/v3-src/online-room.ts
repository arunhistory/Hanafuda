...
export class HanafudaOnlineRoom {
  ...
  async join(body:any){
    ...
    if(!guest.ok||!guest.data?.ok)return json({ok:false,code:"ENGINE_GUEST_SNAPSHOT_FAILED"},502);
  ...
  async connect(req:Request,url:URL){
    ...
    const status=String(await this.state.storage.get("status")??"");if(status==="timeout"||status==="closed")return json({ok:false,code:"ROOM_CLOSED"},410);
    ...
    await this.state.storage.put(seat===0?"disconnectHost":"disconnectGuest",0);await this.scheduleNextAlarm();
  ...
  async onDisconnect(seat:number,socket:any){
    ...
    const status=String(await this.state.storage.get("status")??"");
    if(!["active","round_settlement","complete"].includes(status))return;
    const deadline=Date.now()+60_000;await this.state.storage.put(seat===0?"disconnectHost":"disconnectGuest",deadline);await this.scheduleNextAlarm();
  ...
  async broadcastState(statusOverride?:string){
    ...
    for(const [seat,socket] of this.sockets){
      try{const snap=await this.snapshotFor(seat);socket.send(JSON.stringify({type:"state",status,version,rules,snapshot:snap?.snapshot??null}));}catch{}
    }
  }
  ...
  async action(body:any){
    const status=String(await this.state.storage.get("status")??"");
    if(!["active","round_settlement"].includes(status))return json({ok:false,code:"ROOM_NOT_ACTIONABLE",status},409);
    ...
    if(!Number.isSafeInteger(expected)||expected!==stored)return json({ok:false,code:"VERSION_CONFLICT",version:stored},409);
    ...
    if(roomStatus==="complete")await this.state.storage.put({postmatchChoice:null,postmatchProcessing:false});
  ...
  async postmatch(body:any){
    ...
    if(await this.state.storage.get("postmatchProcessing"))return json({ok:true,locked:true,choice:"processing"});
    ...
    try{
      if(choice==="same"){
        ...
        const created=await engineCall(this.env,{op:"create_internal",rounds:rules.rounds,cpuProfile:0,firstDealer:-1,koiEnabled:rules.koiEnabled});
        if(!created.ok||!created.data?.ok||!created.data?.gameId)throw new Error("REMATCH_CREATE_FAILED");
        ...
        await this.scheduleNextAlarm();this.broadcastSame({type:"postmatch_choice",choice,seat});await this.broadcastState(status);
      ...
      if(choice==="reconfigure"){
        ...
        await this.scheduleNextAlarm();this.broadcastSame({type:"postmatch_choice",choice,seat});return json({ok:true,locked:true,choice,status:"reconfiguring"});
      }
      ...
      await this.scheduleNextAlarm();this.broadcastSame({type:"postmatch_choice",choice,seat});this.closeSockets(1000,"home");return json({ok:true,locked:true,choice,status:"closed"});
  ...
  async reconfigure(body:any){
    ...
    await this.scheduleNextAlarm();this.broadcastSame({type:"rules_changed",rules});await this.broadcastState(status);
    return json({ok:true,status,rules,version:Number(created.data.version),snapshot:created.data.snapshot});
  }

  async close(body:any){
    const seat=await this.authSeat(String(body?.token??""));if(seat<0)return json({ok:false,code:"UNAUTHORIZED"},401);
    await this.state.storage.put({status:"closed",turnSeat:-1,turnDeadline:0,graceDeadline:0,disconnectHost:0,disconnectGuest:0});await this.scheduleNextAlarm();
  ...
  async scheduleNextAlarm(){
    ...
    if(!["active","round_settlement","complete"].includes(status)){await this.state.storage.deleteAlarm();return;}
  ...
  async alarm(){
    const status=String(await this.state.storage.get("status")??"");if(!["active","round_settlement","complete"].includes(status))return;
  ...
  async timeout(reason:string){await this.state.storage.put({status:"timeout",turnSeat:-1,turnDeadline:0,graceDeadline:0,disconnectHost:0,disconnectGuest:0});this.broadcastSame({type:"timeout",reason});this.closeSockets(4000,"timeout");await this.state.storage.deleteAlarm();}
}
