...
const supabaseFx=fs.readFileSync('web/src/supabase-effect-source-v1.ts','utf8');
const finalFix=fs.readFileSync('web/src/final-result-fix-v1.ts','utf8');
const finalCss=fs.readFileSync('web/final-result-fix-v1.css','utf8');
const roundEnd=fs.readFileSync('web/src/round-end-boundary-v1.ts','utf8');
...
const perfSrc=fs.readFileSync('web/src/initial-animation-performance-v1.ts','utf8');
...
for(const name of ['supabase-effect-source-v1','final-result-fix-v1','round-end-boundary-v1','initial-animation-performance-v1']){
  assert.match(html,new RegExp(`\\.\\/dist\\/${name}\\.js`),`${name} must load from generated dist output`);
  assert.doesNotMatch(html,new RegExp(`src="\\.\\/${name}\\.js"`),`${name} handwritten root runtime must not be loaded`);
  assert.ok(fs.existsSync(`web/src/${name}.ts`),`${name} TypeScript source must exist`);
  assert.ok(fs.existsSync(`web/dist/${name}.js`),`${name} generated JavaScript must exist`);
}
...
assert.match(perfSrc,/initial-shuffle-lite/);
assert.match(perfSrc,/await delay\(1080\)/);
assert.match(perfSrc,/initial-deal-stage/);
assert.match(perfSrc,/initial-deal-flight to-opponent/);
assert.match(perfSrc,/initial-deal-flight to-field/);
assert.match(perfSrc,/initial-deal-flight to-player/);
assert.match(perfSrc,/await delay\(620\)/);
assert.match(perfSrc,/__hanafudaInitialAnimationPerformanceVersion="2"/);
