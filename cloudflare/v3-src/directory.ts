...
export async function routeOnlineV3(req:Request,env:any,url:URL){
  if(url.pathname==="/api/online/create"){
    ...
    for(let attempt=0;attempt<8;attempt++){
      ...
      if(!response.ok||!data?.ok)return json(data??{ok:false,code:"ROOM_CREATE_FAILED"},response.status||502);
  ...
  if(url.pathname==="/api/online/join"){
    if(req.method!=="POST")return json({ok:false,code:"METHOD_NOT_ALLOWED"},405);
    let body:any;try{body=await bodyJson(req);}catch(e:any){return json({ok:false,code:e?.message==="REQUEST_TOO_LARGE"?"REQUEST_TOO_LARGE":"INVALID_JSON"},400);}
    const code=String(body?.roomCode??"").toUpperCase();if(!validRoomCode(code))return json({ok:false,code:"INVALID_ROOM_CODE"},400);
    ...
    const data=await response.json().catch(()=>null);
    if(!response.ok||!data?.ok)return json(data??{ok:false,code:"ROOM_JOIN_FAILED"},response.status||502);
  ...
  if(url.pathname==="/api/online/connect"){
    ...
    if(!validRoomCode(code)||!validOpaqueToken(token))return json({ok:false,code:"INVALID_SESSION"},400);
