...
export class HanafudaDirectory {
  ...
  async makeRoom(waitingTicket:string,currentTicket:string,rules:{rounds:number;koiEnabled:boolean}){
    for(let attempt=0;attempt<8;attempt++){
      ...
      const expires=Date.now()+120_000;await this.state.storage.put(`match:${waitingTicket}`,{roomCode:code,token:hostToken,seat:"host",rules,expires});
  ...
  async fetch(req:Request){
    ...
    if(op==="enqueue"){
      ...
      if(waiting&&Number(waiting.expires)>now&&waiting.ticket!==ticket){
        ...
        await this.state.storage.delete(waitKey);await this.state.storage.delete(`ticketrule:${waiting.ticket}`);return json({ok:true,matched:true,...result});
      }
      await this.state.storage.put(waitKey,{ticket,expires:now+120_000});await this.state.storage.put(`ticketrule:${ticket}`,key);return json({ok:true,matched:false,rules});
    }
    if(op==="poll"){
      const match:any=await this.state.storage.get(`match:${ticket}`);if(!match)return json({ok:true,matched:false});await this.state.storage.delete(`match:${ticket}`);await this.state.storage.delete(`ticketrule:${ticket}`);
      if(Number(match.expires)<=Date.now())return json({ok:true,matched:false});return json({ok:true,matched:true,roomCode:match.roomCode,token:match.token,seat:match.seat,rules:match.rules});
    }
    if(op==="cancel"){
      const key=String(await this.state.storage.get(`ticketrule:${ticket}`)??"");if(key){const waitKey=`waiting:${key}`,waiting:any=await this.state.storage.get(waitKey);if(waiting?.ticket===ticket)await this.state.storage.delete(waitKey);}
