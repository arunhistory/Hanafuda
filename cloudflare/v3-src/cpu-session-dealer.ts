...
function parseFirstDealer(v:any){const n=Number(v);return n===-1||n===0||n===1?n:null;}
...
export class HanafudaCpuSessionDealer extends BaseCpuSession {
  async init(body:any){
    ...
    if(!validOpaqueToken(token)||!mode||rounds===null||typeof koiEnabled!=="boolean"||firstDealer===null)return json({ok:false,code:"INVALID_INIT"},400);
...
export async function routeCpuDealer(req:Request,env:any,url:URL){
  if(url.pathname!=="/api/cpu/start")return baseRouteCpu(req,env,url);
  ...
  if(!mode||rounds===null||typeof koiEnabled!=="boolean"||firstDealer===null)return json({ok:false,code:"INVALID_RULESET"},400);
