...
function ensureOrientationGate() {
    let gate = document.querySelector("#orientation-gate");
    if (gate)
        return gate;
...
function applyProfile(profile) {
    ...
    if (mobile)
        ...
        document.querySelector("#orientation-gate")?.remove();
}
async function resolveProfile() {
    try {
        ...
        if (response.ok && (data?.profile === "desktop" || data?.profile === "mobile_landscape"))
            return data.profile;
    }
    catch { }
    return "desktop";
...
function refreshOrientationClass() {
    if (activeProfile !== "mobile_landscape")
        return;
