...
export class HanafudaCpuSession {
    ...
    async init(body) {
        ...
        if (!validOpaqueToken(token) || !mode || rounds === null || typeof koiEnabled !== "boolean")
    ...
    async engineAction(action) {
        ...
        const payload = { op: "action", gameId, seat: 0, expectedVersion: version, ...action };
        const ms = await this.modeSession();
        if (ms)
            payload.modeSession = ms;
        const result = await engineCall(this.env, payload);
        if (result.ok && result.data?.ok && Number.isSafeInteger(Number(result.data.version)))
            await this.state.storage.put("version", Number(result.data.version));
        return result;
    }
    async maybeGrantUnlock(snapshot) {
        if ((await this.state.storage.get("unlockGranted")) === true)
            return true;
        if ((await this.state.storage.get("challenge")) !== true || (await this.state.storage.get("challengeTestOnly")) === true)
            return false;
        if (Number(snapshot?.phase) === 6 && Number(snapshot?.matchWinner) === 0) {
            await this.state.storage.put({ unlockGranted: true, unlockGrantedAt: Date.now() });
            return true;
        }
        return false;
    }
    async runCpu(events) {
        ...
        for (let guard = 0; guard < 8; guard++) {
            ...
            if (!snap || snap.turn !== 1 || !CPU_PHASES.has(Number(snap.phase)))
                return { ok: true };
            let result;
            if (mode === "beginner") {
                const phase = Number(snap.phase);
                const randomIndex = (n) => { const a = new Uint32Array(1); crypto.getRandomValues(a); return n > 0 ? a[0] % n : 0; };
                if (phase === 1) {
                    const count = Math.max(0, Number(snap.opponentHandCount ?? 0));
                    if (count < 1)
                        return { ok: false, response: json({ ok: false, code: "BEGINNER_NO_HAND" }, 500) };
                    result = await this.engineAction({ action: "play", actor: 1, handIndex: randomIndex(count) });
                }
                else if (phase === 2 || phase === 3) {
                    const pending = Array.isArray(snap.pendingMatches) ? snap.pendingMatches.filter((x) => Number.isInteger(Number(x))).map(Number) : [];
                    if (!pending.length)
                        return { ok: false, response: json({ ok: false, code: "BEGINNER_NO_CAPTURE" }, 500) };
    ...
    async action(body) {
        ...
        if (await this.state.storage.get("pendingTransition"))
            return json({ ok: false, code: "TRANSITION_REQUIRED", forcedRounds: Number(await this.state.storage.get("forcedRounds") ?? 6) }, 409);
        ...
        const payload = { action: kind, actor: 0 };
        if (kind === "play")
    ...
    async transition() {
        if ((await this.state.storage.get("closed")) === true)
            return json({ ok: false, code: "SESSION_CLOSED" }, 410);
        if ((await this.state.storage.get("pendingTransition")) !== true)
            return json({ ok: false, code: "NO_PENDING_TRANSITION" }, 409);
        const ms = await this.modeSession();
        if (!ms)
        ...
        let modeId;
        try {
            modeId = this.env.MODE_SESSIONS.idFromString(ms.sessionId);
        }
        catch {
            await engineCall(this.env, { op: "close", gameId: String(created.data.gameId) }).catch(() => null);
            return json({ ok: false, code: "INVALID_MODE_SESSION" }, 409);
        }
        ...
        if (!ack.ok || ackData?.mode !== "impossible") {
            await engineCall(this.env, { op: "close", gameId: String(created.data.gameId) }).catch(() => null);
    ...
    async status() {
        if ((await this.state.storage.get("closed")) === true)
            return json({ ok: true, closed: true, pendingTransition: false, unlockGranted: (await this.state.storage.get("unlockGranted")) === true });
        const gameId = String(await this.state.storage.get("gameId") ?? "");
        const result = await engineCall(this.env, { op: "snapshot", gameId, seat: 0 });
        if (!result.ok || !result.data?.ok)
            return json({ ok: false, code: "ENGINE_STATUS_FAILED" }, result.status || 502);
...
export async function routeCpu(req, env, url) {
    ...
    if (url.pathname === "/api/cpu/start") {
        ...
        if (!mode || rounds === null || typeof koiEnabled !== "boolean")
    ...
    let id;
    try {
        id = env.CPU_SESSIONS.idFromString(sessionId);
    }
    catch {
        return json({ ok: false, code: "INVALID_SESSION" }, 400);
    }
